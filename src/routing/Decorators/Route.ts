...
export function Route(
	...
	return function (constructor: Type<any>) {
		...
		metadata.RegisterHttpRoute(
			new HttpRouteMetadata(constructor, router, normalizedEndpoint)
		);
		...
		metadata.GetEndpoints(constructor).forEach((httpEndpointMetadata) => {
			const normalizedEndpoint =
				httpEndpointMetadata.endpoint instanceof RegExp
					? httpEndpointMetadata.endpoint
					: path.normalize(path.join("/", httpEndpointMetadata.endpoint));
			const endpointHandler = async function () {
				const [request, response] = Array.from(arguments) as [
					Request,
					Response
				];
					...
					switch (parameterMetadata.type) {
						case ParameterType.FROM_HEADER:
							...
							if (typeof headerArgument === "function") {
								...
								header =
									request.header(headerArgument) ??
									request.headers[headerArgument];
							}
						...
						case ParameterType.REQUEST:
							if (parameterMetadata.payload) {
								parameters[parameterMetadata.index] =
									parameterMetadata.payload(request);
							} else {
						...
						case ParameterType.FROM_QUERY:
							...
							if (parameterMetadata.payload) {
								...
								if (userDefinedType(parameterMetadata.expectedType)) {
									parameters[parameterMetadata.index] = await construct(
										parameterMetadata.expectedType as Type<any>,
										query as any
									);
								} else {
						...
						case ParameterType.FROM_SERVICES:
							parameters[parameterMetadata.index] = request.inject(
								parameterMetadata.expectedType
							);
							break;
						default:
							const incomingPayload = request[parameterMetadata.type];
							parameters[parameterMetadata.index] = userDefinedType(
								parameterMetadata.expectedType
							)
								? await construct(
										parameterMetadata.expectedType as Type<any>,
			...
			const routeMiddlewares = metadata.GetHttpEndpointMiddlewares(
				httpEndpointMetadata.getHandlerName()
			);
			const authorizeMiddlewares =
				metadata.GetAuthorizeMiddlewares(httpEndpointMetadata);
			router[httpEndpointMetadata.method](
				normalizedEndpoint,
				autoHandler(
					...authorizeMiddlewares, // FIXME: should be removed after adding RequireAuthenticatedUser
					...(authorizeMiddlewares.length
